...
import { SwarmMarketsToken, SmtPriceFeed, BPoolMock, BRegistryMock, EurPriceFeedMock, XTokenWrapperMock, ChainlinkAggregatorMock, ERC20Detailed } from '../typechain';
  ...
  describe('getPrice - #calculateAmount', () => {
    describe('when there are pools containing asset/SMT pair', () => {
      it('should get the right amount (avg from the 4 pools)', async () => {
        expect((await bRegistryContract.getBestPoolsWithLimit('0xC34Fb71AAc05d01F354108Ac6ff956f8c5deAeF4', ethers.constants.AddressZero, 0)).length).to.eq(4)
        expect(await bRegistryContract.emptyOnAsset()).to.eq(false)
        const tokenAmountIn = ethers.constants.WeiPerEther.mul(4)

        const smtAmount = await smtPriceFeedContract.calculateAmount('0xC34Fb71AAc05d01F354108Ac6ff956f8c5deAeF4', tokenAmountIn)
      ...
      it('should get the right amount (from pool1)', async () => {
        ...
        expect((await bRegistryContract.getBestPoolsWithLimit('0xC34Fb71AAc05d01F354108Ac6ff956f8c5deAeF4', ethers.constants.AddressZero, 0)).length).to.eq(1)
        const tokenAmountIn = ethers.constants.WeiPerEther.mul(4)
      ...
      it('should get the right price (avg from the 4 pools)', async () => {
        await reverter.revert();
        expect((await bRegistryContract.getBestPoolsWithLimit(xToken.address, ethers.constants.AddressZero, 0)).length).to.eq(4)
        expect(await bRegistryContract.emptyOnAsset()).to.eq(false)
      ...
      it('should get the right price (from pool1)', async () => {
        expect(await bRegistryContract.emptyOnAsset()).to.eq(false)
        await bRegistryContract.setPools([pool1.address])
        expect((await bRegistryContract.getBestPoolsWithLimit(xToken.address, ethers.constants.AddressZero, 0)).length).to.eq(1)
        const one = ethers.BigNumber.from(1000000) // 1 USDC
    ...
    describe('when there are pools containing asset/SMT pair', () => {
      ...
      beforeEach(async () => {
        ...
        expect((await bRegistryContract.getBestPoolsWithLimit('0xC34Fb71AAc05d01F354108Ac6ff956f8c5deAeF4', ethers.constants.AddressZero, 0)).length).to.eq(0)
        expect((await bRegistryContract.getBestPoolsWithLimit('0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE', ethers.constants.AddressZero, 0)).length).to.eq(4)
      })

      it('should return 0 if assetEthFeed is zero address', async () => {
        expect(await eurPriceFeedContract.assetEthFeed('0xC34Fb71AAc05d01F354108Ac6ff956f8c5deAeF4')).to.eq(ethers.constants.AddressZero)

        const smtAmount = await smtPriceFeedContract.calculateAmount('0xC34Fb71AAc05d01F354108Ac6ff956f8c5deAeF4', ethers.constants.WeiPerEther)
      ...
      it('should get the right amount using the avg from the 4 eth/smt pools and external asset feed)', async () => {
        ...
        const ethToSmtAmount = (ethers.constants.WeiPerEther.add(ethers.constants.WeiPerEther.div(2)).add(ethers.constants.WeiPerEther.div(3)).add(ethers.constants.WeiPerEther.div(4))).div(4)
      ...
      it('should return 0 if there are no eth/smt pool', async () => {
        ...
        expect((await bRegistryContract.getBestPoolsWithLimit('0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE', ethers.constants.AddressZero, 0)).length).to.eq(0)
        const tokenAmountIn = ethers.BigNumber.from(100000) // 0.1 USDC
